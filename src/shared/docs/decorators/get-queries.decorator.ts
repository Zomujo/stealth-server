...
export const QueryDocs = (params: QueryParams[]) => {
  ...
  params.forEach((param) => {
    switch (param) {
      case 'search':
        docs.push(ApiQuery({ name: 'search', type: String, required: false, description: 'what to search for' }));
        break;
      case 'limit':
        docs.push(ApiQuery({ name: 'limit', type: Number, required: false, description: 'The maximum number of rows to retrieve' }));
        break;
      case 'page':
        docs.push(ApiQuery({ name: 'page', type: Number, required: false, description: 'The page number' }));
        break;
    }
  })
